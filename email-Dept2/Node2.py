...
def zhenghe_data():
    ...
    DF.to_csv("importance_dept2.csv", index=False)


# 先删除 𝐾𝑠 = 0且Salton指标为0的节点，再删除权重为零的节点
def shuju_quzao():
    df = pd.read_csv('importance_dept2.csv')
    qu_kshell = df[(df['kshell'] != 0.0)]  # 144
    qu_salton = qu_kshell[(qu_kshell['salton'] != 0.0)]  # 144
    qu_quanzhong = qu_salton[(qu_salton['quanzhong'] != 0.0)]  # 113
...
def Monotonicity():
    ...
    print(result)  # 0.9980704297789239
...
def attribute(G):
    ...
    print("email-dept2网络的节点聚类系数和为：" + str(result))
    avg_result = result / 162
    print("email-dept2网络的平均聚类系数和为：" + str(avg_result))
    print(round(avg_result, 4))
    r = nx.degree_pearson_correlation_coefficient(G, weight=None, nodes=None)
    print("email-dept2网络的同配系数为：" + str(r))
    print(round(r, 4))
