...
def importance_node():
    ...
    #     DF['importance'] = df1['kshell'] + (df2['neighbour_bc'] * df4['BC'] * all_salton[j]) / (df5['CC'] * df5['CC'])
    # DF.to_csv("result_importance.csv", index=False)
    # print('计算节点重要性结束')


def importance_node_sort():
    df = pd.read_csv('result_important.csv')
...
def zhenghe_data():
    ...
    DF.to_csv("importance_dept1.csv", index=False)


# 先删除 𝐾𝑠 = 0且Salton指标为0的节点，再删除权重为零的节点
def shuju_quzao():
    df = pd.read_csv('importance_dept1.csv')
    qu_kshell = df[(df['kshell'] != 0.0)]  # 272
    qu_salton = qu_kshell[(qu_kshell['salton'] != 0.0)]  # 272
    qu_quanzhong = qu_salton[(qu_salton['quanzhong'] != 0.0)]  # 231
...
def Monotonicity():
    ...
    output = []
    similar = 0
    for i in (0.064064, 0.250011, 0.250049, 0.25007, 0.250379, 0.562574, 0.761355, 0.824665, 1.010562):
        li = [i]
    ...
    for j in range(0, 9):
        ...
        # print(tar)
    print(similar)
    result = pow((1 - (similar / (230 * 229))), 2)
    print(result)  # 0.9991647843040026
...
def attribute(G):
    ...
    print("email-dept1网络的节点聚类系数和为：" + str(result))
    avg_result = result / 309
    print("email-dept1网络的平均聚类系数和为：" + str(avg_result))
    print(round(avg_result, 4))
    r = nx.degree_pearson_correlation_coefficient(G, weight=None, nodes=None)
    print("email-dept1网络的同配系数为：" + str(r))
    print(round(r, 4))
    L = nx.average_shortest_path_length(G, weight=None, method=None)
    print("email-dept1网络的同配系数为：" + str(L))
    print(round(L, 4))
    g_average_path_len = nx.average_shortest_path_length(G)
    print("平均路径长度：" + str(g_average_path_len))
