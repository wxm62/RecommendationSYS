...
def sort_kshell():
    ...
    # 不保留六位小数
    DF1['kshell_guiyihua'] = ((df1['kshell'] - 1) / (20 - 1))
...
def zhenghe_data():
    ...
    DF.to_csv("importance_facebook.csv", index=False)


# 先删除 𝐾𝑠 = 0且Salton指标为0的节点，再删除权重为零的节点
def shuju_quzao():
    df = pd.read_csv('importance_facebook.csv')
    qu_kshell = df[(df['kshell'] != 0.0)]  # 300
    qu_salton = qu_kshell[(qu_kshell['salton'] != 0.0)]  # 300
    qu_quanzhong = qu_salton[(qu_salton['quanzhong'] != 0.0)]  # 269
...
def Monotonicity():
    ...
    output = []
    similar = 0
    for i in (0.974819, 0.614537, 0.51416, 0.210631, 0.210607, 0.191344, 0.157948, 0.062554):
        li = [i]
    ...
    for j in range(0, 8):
        ...
        # print(tar)
    print(similar)
    result = pow((1 - (similar / (268 * 267))), 2)
    print(result)  # 0.9991057956072186
...
def attribute(G):
    ...
    print("Facebook网络的节点聚类系数和为：" + str(result))
    avg_result = result / 333
    print("Facebook网络的平均聚类系数和为：" + str(avg_result))
    print(round(avg_result, 4))
    r = nx.degree_pearson_correlation_coefficient(G, weight=None, nodes=None)
    print("Facebook网络的同配系数为：" + str(r))
    print(round(r, 4))
