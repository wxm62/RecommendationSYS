...
def sort_kshell():
    ...
    # 不保留六位小数
    DF1['kshell_guiyihua'] = ((df1['kshell'] - 1) / (15 - 1))
...
def zhenghe_data():
    ...
    DF.to_csv("importance_dept4.csv", index=False)


# 先删除 𝐾𝑠 = 0且Salton指标为0的节点，再删除权重为零的节点
def shuju_quzao():
    df = pd.read_csv('importance_dept4.csv')
    qu_kshell = df[(df['kshell'] != 0.0)]  # 131
    qu_salton = qu_kshell[(qu_kshell['salton'] != 0.0)]  # 131
    qu_quanzhong = qu_salton[(qu_salton['quanzhong'] != 0.0)]  # 105
...
def Monotonicity():
    ...
    output = []
    similar = 0
    for i in (1.01341, 0.870267, 0.861451, 0.438169):
        li = [i]
    ...
    print(result)  # 0.9992010386816434
...
def attribute(G):
    ...
    print("email-dept4网络的节点聚类系数和为：" + str(result))
    avg_result = result / 142
    print("email-dept4网络的平均聚类系数和为：" + str(avg_result))
    print(round(avg_result, 4))
    r = nx.degree_pearson_correlation_coefficient(G, weight=None, nodes=None)
    print("email-dept4网络的同配系数为：" + str(r))
    print(round(r, 4))
